...
import static com.example.utils.Constant.RESULT_JSON_PATH;
...
//        RestTemplate restTemplate = builder.build();
    ...
    public static void receiveResult(@RequestBody String stringStatistics) throws IOException {
        System.out.println("#Service2-receiveReport: Service 2 received POST-request, receiving result.json from Service 3...");

        Path pathToReport = Paths.get(RESULT_JSON_PATH);

        /**
         * write to result.json
         */
        ObjectWriter objectWriterTimeTable = new ObjectMapper().writer().withDefaultPrettyPrinter();
        Writer writer = new FileWriter(String.valueOf(pathToReport));
        objectWriterTimeTable.writeValue(writer, stringStatistics);
